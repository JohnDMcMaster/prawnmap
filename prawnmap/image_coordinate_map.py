...
class ImageCoordinatePair:
    ...
    def from_spatial_points(first, second):
        return ImageCoordinatePair(
            ImageCoordinateMapPairing(first.coordinates[1],
                                      first.coordinates[0]),
            ImageCoordinateMapPairing(second.coordinates[1],
...
def get_row_col(file_name):
    ...
    if len(parts) != 2:
        raise Exception(
            'Expect files named like cXXXX_rXXXX.tif for automagic stitching, got %s'
            % file_name)
...
class ImageCoordinateMap:
    ...
    def get_image(self, col, row, default=None):
        if col < 0 or col >= self.cols or row < 0 or row >= self.rows:
            raise IndexError(
                'col %d row %d out of range for width %d height %d' %
                (col, row, self.width(), self.height()))
        return self.layout.get((col, row), default)

    def get_images_from_pair(self, pair):
        # ImageCoordinatePair
        return (self.get_image(pair.first.col, pair.first.row),
                self.get_image(pair.second.col, pair.second.row))

    def set_image_rc(self, row, col, file_name, check_bounds=True):
        if check_bounds and (row >= self.height() or col >= self.width()
                             or row < 0 or col < 0):
            raise Exception(
    ...
    def from_dir_tagged_file_names(dir, rows=None, cols=None):
        return ImageCoordinateMap.from_tagged_file_names(
            [os.path.join(dir, f) for f in os.listdir(dir)], rows, cols)

    @staticmethod
    def from_tagged_file_names(file_names,
                               rows=None,
                               cols=None,
                               partial=False,
                               check_bounds=True):
        ...
        for file_name in file_names:
            ...
            ret.set_image_rc(row, col, file_name, check_bounds=check_bounds)
            ...
            for col_1 in range(max(0, col_0 - col_spread),
                ...
                for row_0 in range(0, self.rows):
                    ...
                    for row_1 in range(max(0, row_0 - row_spread),
                        ...
                        to_yield = ImageCoordinatePair(
                            ImageCoordinateMapPairing(col_1, row_1),
                            ImageCoordinateMapPairing(col_0, row_0))
                        yield to_yield
